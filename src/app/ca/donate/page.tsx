...
export default function DonatePageCanada() {
  const [loading, setLoading] = useState(false);
  const { toast } = useToast();

  const donationAccountId =
    process.env.NEXT_PUBLIC_CA_DONATION_ACCOUNT_ID || process.env.NEXT_PUBLIC_DONATION_ACCOUNT_ID || '';
  const donationDestinationLabel =
    process.env.NEXT_PUBLIC_CA_DONATION_DESTINATION_LABEL || process.env.NEXT_PUBLIC_DONATION_DESTINATION_LABEL || '';
    ...
    <>
      <section className="relative h-[40vh] min-h-[300px] w-full bg-primary/20 flex items-center justify-center">
        ...
        <div className="relative z-10 text-center text-white p-4">
          <h1 className="font-headline text-4xl md:text-5xl font-bold">Support Our Work in Canada</h1>
          <p className="mt-2 max-w-2xl text-lg text-primary-foreground">
            Your generosity helps strengthen health equity, inclusion, and community wellbeing.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-2xl">
          <Card className="mb-6 border bg-background/70">
            ...
            <CardContent className="text-sm text-muted-foreground space-y-2">
              ...
              <p>
                ...
                {donationDestinationLabel ? <span className="text-foreground">{donationDestinationLabel}</span> : 'Contact us for details.'}
              </p>
              <p>
                Email:{' '}
                <a className="text-primary hover:underline" href="mailto:info@seqher.org?subject=Donation%20Support%20(Canada)">
                  info@seqher.org
                </a>
              </p>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Every contribution helps us expand access and support communities.</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
