...
export const metadata = {
  title: 'Careers | SEQHER Canada',
  description: 'Explore job openings and opportunities at SEQHER in Canada.',
};
...
function extractJobMeta(content: string) {
  ...
  const pick = (label: string) => {
    const re = new RegExp(
      `${label}:\\s*([^]+?)(?=\\s+(?:Position Title:|Location:|Contract Duration:|Release Date:|Application Deadline:|About SEQHER|Position Summary|Key Responsibilities|Required Qualifications|Preferred Candidate Profile|How to Apply)\\b|$)`,
      'i'
    );
...
function isCanadaJob(location: string | null) {
  if (!location) return false;
  const normalized = location.toLowerCase();
  if (normalized.includes('canada')) return true;
  return ['toronto', 'ottawa', 'montreal', 'montréal', 'vancouver', 'calgary'].some((city) => normalized.includes(city));
}
...
export default function CareersPage() {
  ...
  const careerJobs: CareerJob[] = jobs
    ...
    .filter((job) => isCanadaJob(job.meta.location));
  ...
  return (
    <div className="bg-background">
      <section className="relative h-[45vh] min-h-[320px] w-full bg-primary/15 flex items-center justify-center">
        <Image src={heroImage} alt="SEQHER careers (Canada)" fill className="object-cover" sizes="100vw" priority />
        <div className="absolute inset-0 bg-black/50" />
        <div className="relative z-10 text-center text-white p-4">
          <p className="uppercase tracking-[0.3em] text-xs md:text-sm">Join the Mission</p>
          <h1 className="font-headline text-4xl md:text-5xl font-bold mt-3">Careers at SEQHER (Canada)</h1>
          <p className="mt-3 max-w-3xl mx-auto text-primary-foreground text-lg">
            {careerJobs.length ? (
              ...
              <>No Canada-based openings are listed right now. Please check back soon.</>
            )}
          </p>
        </div>
      </section>
      ...
      <section className="py-12 md:py-16">
        <div className="container mx-auto px-4 max-w-5xl space-y-8">
          <CareersClient jobs={careerJobs} fullPageHrefBase={null} />
        </div>
      </section>
    </div>
  );
